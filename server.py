...
def profile():
    ...
    if flask.request.method == "GET":
        ...
        flask.request.method == "POST" and "confirm-new-password" in flask.request.form
    ):
        if not user.password.check_password(flask.request.form.get("old-password")):
            flask.flash("Incorrect password")

        elif not flask.request.form.get("new-password") == flask.request.form.get(
    ...
    # Importing an opml file
        ...
        user = User.get(email=flask.session.get("email"))
        for youtube_id in re.findall(
            r'xmlUrl="https://www.youtube.com/feeds/videos.xml\?channel_id=(.*?)"',
            file.read().decode(),
        ):
...
def get_subscriptions():
    ...
    # Adding a new subscription
        ...
        try:
            ...
            flask.flash(
                f'Unable to add {flask.request.form["id_or_title"]}, could not find subscription'
            )
            return flask.redirect("/subscriptions")
...
def get_single_subscription(youtube_id):
    ...
    elif flask.request.method == "DELETE" or (
        flask.request.method == "POST" or "delete" in flask.request.args
    ):
        user = User.get(email=flask.session.get("email"))
...
def get_single_filters(id):
    ...
    if flask.request.method == "GET":
        ...
        flask.request.method == "POST" or "action-delete" in flask.request.form
    ):
        filter = Filter.get(id=id)
